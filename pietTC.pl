...
darkdiff(black,black,_).
darkdiff(white,white,_).
...
stepdiff(C1, C2, D) :- hue(C1,H1), hue(C2,H2), D is (H2 - H1) mod 8 .
stepdiff(black,black,_).
stepdiff(white,white,_).
...
operation(ojump, 6,0).
operation(ohuechange, 6,1).
operation(olightchange, 6,2).
operation(o2white, 7,0).
operation(o2black, 7,1).
operation(o2color, 7,2).
...
applyOp(ojump,_) :- retract(pile([X,Y|H])), retract(curs(_,_)), assert(curs(X,Y)), assert(pile(H)),!.
applyOp(ohuechange,_) :- 
  retract(pile([X,Y,V|H])), 
  retract(program(X,Y,C1)), 
  stepdiff(C1,C2,V), 
  darkdiff(C1,C2,0),
  assert(program(X,Y,C2)), 
  assert(pile(H)),!.
applyOp(olightchange,_) :- 
  retract(pile([X,Y,V|H])), 
  retract(program(X,Y,C1)),
  stepdiff(C1,C2,0), 
  darkdiff(C1,C2,V),
...
applyOp(o2white,_) :- 
  ...
  assert(program(X,Y,white)),
  assert(pile(H)),!.
applyOp(o2black,_) :- 
  retract(pile([X,Y|H])), 
  retract(program(X,Y,_)),
  assert(program(X,Y,black)),
  assert(pile(H)),!.
applyOp(o2color,_) :- 
